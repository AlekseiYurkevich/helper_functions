...
from help_functions.utils.image_utils import load_grayscale
from help_functions.utils.metrics import (
...
def confusion_matrix_from_files(
    y_true_path: Path, y_pred_path: Path, num_classes: int
) -> np.array:
...
def calculate_ious_global(
    y_true_path: Path, y_pred_path: Path, num_classes: int, num_workers: int = 1
) -> np.array:
    ...
    def helper(file_name):
        return confusion_matrix_from_files(
            y_true_path.joinpath(file_name),
            y_pred_path.joinpath(file_name),
            num_classes,
        )

    matrices = Parallel(n_jobs=num_workers)(
        delayed(helper)(file_path.name) for file_path in tqdm(y_pred_files)
    )
...
def get_args():
    ...
    arg(
        ...
        "--ground_truth_path",
        type=Path,
        help="Json with ground truth.",
        required=True,
    )
    arg(
        "-n", "--num_classes", type=int, help="Number of classes to use.", required=True
    )
