...
@given(
    numerical=h_arrays(
        dtype=float, shape=(ARRAY_SHAPE, 3), elements=h_float(-MIN_VALUE, MAX_VALUE)
    )
)
...
@given(
    cyclical=h_arrays(
        dtype=float, shape=(ARRAY_SHAPE, 5), elements=h_float(-MIN_VALUE, MAX_VALUE)
    )
)
...
def test_encoder_categorical(categorical):
    ...
    assert set(transformed.keys()) == {category_type}
    assert set(transformed.keys()).intersection(columns_map.keys()) == set(
        transformed.keys()
    )
...
def test_encoder(numerical, cyclical, categorical):
    ...
    assert set(columns_map.keys()) == set(
        transformed.keys()
    ), f"{transformed.keys()} {columns_map.keys()}"
