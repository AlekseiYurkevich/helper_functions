...
def cross_validation_score_statement(
    estimator, X, y, scoring, n_splits=5, statement=None, random_state=0
):
    """
    ...
    for train, test in cv_iter:
        ...
        if statement is not None:
            ...
            pred_statement = estimator.predict(
                X.iloc[test, :].loc[statement[test]].values
            )
        else:
...
def cross_validation_score_fit_subset(
    estimator, X, y, scoring, n_splits=5, statement=None, random_state=0
):
    """
    Evaluate a score by cross-validation. 
    ...
    for train, test in cv_iter:
        if statement is not None:
            estimator.fit(
                X.iloc[train, :].values[statement[train]],
                y.iloc[train].values[statement[train]],
            )
        else:
            estimator.fit(X.iloc[train, :].values, y.iloc[train].values)
        y_ = y.iloc[test]
