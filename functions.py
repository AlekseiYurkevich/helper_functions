...
def cont_split(data, feature, target, leafs, auto_calc=False):
    ...
    if auto_calc == True:
        temp_series = pd.DataFrame(
            {
                feature: pd.cut(data[feature], bins=[v for v in d.values()]),
                target: data[target],
            }
        )
    ...
    print(
        pd.cut(x, bins=[v for v in d.values()]).value_counts(
...
def calc_iv(data, target, feature):
    df = pd.DataFrame(
        index=data[feature].unique(),
        data={
            "% responders": data.groupby(feature)[target].sum() / np.sum(data[target])
        },
    )
    df["% non-responders"] = (
        data.groupby(feature)[target].count() - data.groupby(feature)[target].sum()
